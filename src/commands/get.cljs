      ...
      (let [server (.-guild interaction)
        ...
        (if player-server
          (let [player-info (db-utils/get-first-formatted-row (<p! (player/select-player user-id)))
            ...
            (fill-text "Tag" 240 56)
            (make-context-second-column)
            (fill-text (.toLowerCase (player-info "tag")) 318 56)

            (make-context-first-column)
            (fill-text "Points" 240 91)
            (make-context-second-column)
            (fill-text (.toFixed player-points 2) 318 91)

            (make-context-first-column)
            (fill-text "Wins" 498 56)
            (make-context-second-column)
            (fill-text player-total-wins 579 56)

            (make-context-first-column)
            (fill-text "Losses" 498 91)
            (make-context-second-column)
            (fill-text player-total-losses 579 91)

            (make-context-first-column)
            (fill-text "Draws" 498 126)
            (make-context-second-column)
            (fill-text player-total-draws 579 126)

            (make-context-first-column)
            (fill-text "Total" 498 161)
            (make-context-second-column)
            (fill-text player-total-matches 579 161)

            (make-context-first-column)
            (fill-text "Win Rate" 498 244)
            (make-context-second-column)
            (fill-text (str (.toFixed (if (js/isNaN player-win-rate) 0 player-win-rate)) "%")
                       605 244)

            (fill-style rank-color)
            (fill-text (str "\"" rank-name"\"") 240 244)
